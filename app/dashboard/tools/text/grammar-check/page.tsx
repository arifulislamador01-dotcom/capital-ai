...
export default function GrammarCheckPage() {
  const tool = getToolById('grammar-check');
  ...
  const handleGenerate = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch('/api/text/grammar', {
        method: 'POST',
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      ...
      <div className="glass-card p-6 space-y-6 border border-slate-800">
        <div>
          <label className="block text-sm mb-2 text-slate-300">ভুলসহ টেক্সট দিন</label>
          <textarea value={prompt} onChange={e => setPrompt(e.target.value)} placeholder="He go to school..." className="w-full bg-slate-900 border border-slate-700 rounded-xl p-4 text-white focus:border-cyan-500 focus:outline-none min-h-[120px]" />
        </div>
        ...
        <button onClick={handleGenerate} disabled={loading} className="w-full py-4 rounded-xl font-bold text-lg bg-gradient-to-r from-cyan-500 to-purple-600 text-white shadow-[0_0_20px_rgba(0,212,255,0.4)] flex items-center justify-center gap-2">
          {loading ? <span className="animate-spin text-xl">↻</span> : <><CheckSquare className="w-5 h-5"/> গ্রামার চেক করুন</>}
        </button>
      </div>

      {loading && <LoadingCard text="গ্রামার চেক হচ্ছে..." />}

      {result && (
        <div className="glass-card p-6 animate-slide-up border border-slate-800">
          <h3 className="text-lg font-bold mb-4 text-cyan-400">সঠিক লেখা:</h3>

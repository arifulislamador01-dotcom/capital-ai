...
export async function POST(req: Request) {
  try {
    ...
    const rl = await checkRateLimit(userId, 'caption', plan.planType);
    if (!rl.allowed) return NextResponse.json({ success: false, error: rl.errorBn }, { status: 429 });

    const { prompt } = await req.json();
    if (!prompt) return NextResponse.json({ success: false, error: 'বিবরণ দিন!' }, { status: 400 });

    const apiKey = process.env.GEMINI_API_KEY;
    if (!apiKey) return NextResponse.json({ success: false, error: 'API Key নেই!' }, { status: 500 });

    const fullPrompt = `Generate a catchy social media caption with emojis and hashtags based on: ${prompt}`;
